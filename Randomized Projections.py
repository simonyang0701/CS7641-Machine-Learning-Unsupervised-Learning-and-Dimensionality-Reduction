...
rp = random_projection.GaussianRandomProjection(n_components = X1.shape[1])

pipe = Pipeline(steps=[('rp', rp), ('neural networks', nn)])

# Plot the Randomized Projections spectrum
rp.fit(X1)
...
rp_new = random_projection.GaussianRandomProjection(n_components = gridSearch.best_estimator_.named_steps['rp'].n_components)
rp_new.fit(X1)
X_transformed_f = rp_new.transform(X1)
...
reconstruction_error_rp_cancer = np.concatenate((np.arange(1,X1.shape[1]).reshape(-1,1), reconstruction_error), axis = 1)

file = './outputs/data_em_rp_nba.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_em_rp_cancer)

file = './outputs/data_km_rp_nba.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_km_rp_cancer)

file = './outputs/reconstruction_error_rp_nba.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
...
nn = MLPClassifier(hidden_layer_sizes=(30,), solver='adam', activation='logistic', learning_rate_init=0.05, 
                                max_iter=1000, random_state=100)
rp = random_projection.GaussianRandomProjection(n_components = X2.shape[1])
...
rp_new = random_projection.GaussianRandomProjection(n_components = gridSearch.best_estimator_.named_steps['rp'].n_components)
rp_new.fit(X2)
X_transformed_f = rp_new.transform(X2)
...
reconstruction_error_rp_cancer = np.concatenate((np.arange(1,X2.shape[1]).reshape(-1,1), reconstruction_error), axis = 1)

file = './outputs/data_em_rp_lol.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_em_rp_cancer)

file = './outputs/data_km_rp_lol.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(data_km_rp_cancer)

file = './outputs/reconstruction_error_rp_lol.csv'
with open(file, 'w', newline = '') as output:
	writer = csv.writer(output, delimiter=',')
	writer.writerows(reconstruction_error_rp_cancer)
