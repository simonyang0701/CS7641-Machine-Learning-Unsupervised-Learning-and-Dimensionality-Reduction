...
pipe = Pipeline(steps=[('pca', pca), ('neural networks', nn)])

# Plot the PCA spectrum
pca.fit(X1)

fig, ax = plt.subplots()
ax.bar(np.arange(X1.shape[1]), pca.explained_variance_ratio_, linewidth=2, color = 'blue')
plt.axis('tight')
...
pca_new = PCA(n_components = gridSearch.best_estimator_.named_steps['pca'].n_components)
pca_new.fit(X1)
X_transformed_f = pca_new.transform(X1)
...
reconstruction_error_pca_cancer = np.concatenate((np.arange(1,X1.shape[1]).reshape(-1,1), reconstruction_error), axis = 1)

file = './outputs/data_em_pca_nba.csv'
with open(file, 'w', newline = '') as output:
    writer = csv.writer(output, delimiter=',')
    writer.writerows(data_em_pca_cancer)

file = './outputs/data_km_pca_nba.csv'
with open(file, 'w', newline = '') as output:
    writer = csv.writer(output, delimiter=',')
    writer.writerows(data_km_pca_cancer)

file = './outputs/reconstruction_error_pca_nba.csv'
with open(file, 'w', newline = '') as output:
    writer = csv.writer(output, delimiter=',')
...
ax.bar(np.arange(X2.shape[1]), pca.explained_variance_ratio_, linewidth=2, color = 'blue')
plt.axis('tight')
plt.xlabel('n_components')
ax.set_ylabel('explained variance ratio')
...
pca_new = PCA(n_components = gridSearch.best_estimator_.named_steps['pca'].n_components)
pca_new.fit(X2)
X_transformed_f = pca_new.transform(X2)
...
reconstruction_error_pca_cancer = np.concatenate((np.arange(1,X2.shape[1]).reshape(-1,1), reconstruction_error), axis = 1)

file = './outputs/data_em_pca_lol.csv'
with open(file, 'w', newline = '') as output:
    writer = csv.writer(output, delimiter=',')
    writer.writerows(data_em_pca_cancer)

file = './outputs/data_km_pca_lol.csv'
with open(file, 'w', newline = '') as output:
    writer = csv.writer(output, delimiter=',')
    writer.writerows(data_km_pca_cancer)

file = './outputs/reconstruction_error_pca_lol.csv'
with open(file, 'w', newline = '') as output:
    writer = csv.writer(output, delimiter=',')
    writer.writerows(reconstruction_error_pca_cancer)
